...
class AppWidgets:
    def __init__(self, config_filename):
        ...
        self.samples_collection_openbis_path = self.config[
            "samples_collection_openbis_path"
        ]
        ...
        self.openbis_session, self.session_data = utils.connect_openbis(
            self.config_eln["url"], self.config_eln["token"]
        )
        ...
        if self.openbis_session:
            self.openbis_connection_status_htmlbox.value = self.config["home_page"][
                "enable_status"
            ]
            self.open_notebooks_html_enable_code = "".join(
                self.config["home_page"]["enable_links"]
            )
            self.open_notebooks_htmlbox = utils.HTMLbox(
                value=self.open_notebooks_html_enable_code
            )
        else:
            self.openbis_connection_status_htmlbox.value = self.config["home_page"][
                "disable_status"
            ]
            self.open_notebooks_html_disable_code = "".join(
                self.config["home_page"]["disable_links"]
            )
            self.open_notebooks_htmlbox = utils.HTMLbox(
                value=self.open_notebooks_html_disable_code
            )

        # Get slabs types
        self.slabs_types = [
            object_info["openbis_object_type"]
            for _, object_info in self.config["objects"].items()
            if object_info["object_type"] == "slab"
        ]

        # Get sample preparation types
        self.sample_preparation_sample_types = [
            object_info["openbis_object_type"]
            for _, object_info in self.config["objects"].items()
            if object_info["object_type"] == "sample_preparation"
        ]
        self.sample_preparation_options = [
            object_key
        ...
        self.cancel_new_experiment_button = utils.Button(
            ...
            tooltip="Cancel",
            icon="times",
            layout=ipw.Layout(width="50px", height="35px", margin="0 0 0 5px"),
        )
        self.new_experiment_name_textbox = utils.Text(
            description="Name",
            disabled=False,
            layout=ipw.Layout(width="400px"),
            placeholder="Write experiment name here...",
        ...
        self.material_selection_radio_button = utils.Radiobuttons(
            description="",
        ...
        self.materials_dropdown = utils.Dropdown(
            description="", disabled=False, layout=ipw.Layout(width="350px")
        )
        self.material_details_textbox = utils.Textarea(
            description="",
            disabled=True,
            layout=ipw.Layout(width="425px", height="200px"),
        )
        self.material_image_box = utils.Image(
            value=utils.read_file(self.config["default_image_filepath"]),
            format="jpg",
            width="200px",
            height="300px",
            layout=ipw.Layout(border="solid 1px #cccccc"),
        )
        self.material_metadata_boxes = ipw.HBox(
            [self.material_details_textbox, self.material_image_box]
        )
        self.material_sorting_checkboxes = utils.SortingCheckboxes(
            "50px", "60px", "200px"
        )

        # Get widgets for sample dropdown
        self.samples_dropdown_boxes = utils.DropdownwithSortingCheckboxesWidget(
            "Sample", ipw.Layout(width="385px"), {"description_width": "110px"}, [-1]
        )
        self.sample_details_textbox = utils.Textarea(
            description="",
            disabled=True,
            layout=ipw.Layout(width="589px", height="300px"),
        )
        self.sample_metadata_boxes = ipw.HBox(
            [self.samples_dropdown_boxes, self.sample_details_textbox]
        )

        # Get widgets for instrument dropdown
        self.instruments_dropdown_boxes = utils.DropdownwithSortingCheckboxesWidget(
            "Instrument",
            ipw.Layout(width="982px"),
            {"description_width": "110px"},
            [-1],
        )

        # Get widgets for project dropdown
        self.projects_dropdown_boxes = utils.DropdownwithSortingCheckboxesWidget(
            "Project", ipw.Layout(width="982px"), {"description_width": "110px"}, [-1]
        )

        # Get widgets for experiment dropdown
        self.experiments_dropdown = utils.Dropdown(
        ...
        self.experiment_sorting_checkboxes = utils.SortingCheckboxes(
            "130px", "60px", "200px"
        )
        self.experiments_dropdown_details = ipw.HBox(
            [self.experiments_dropdown, self.create_new_experiment_button]
        )
        self.experiments_dropdown_boxes = ipw.VBox(
            [self.experiments_dropdown_details, self.experiment_sorting_checkboxes]
        )

        # Get widgets for substance dropdown
        self.substances_dropdown_boxes = utils.DropdownwithSortingCheckboxesWidget(
            "Substance", ipw.Layout(width="335px"), {"description_width": "110px"}, [-1]
        )
        self.substance_details_textbox = utils.Textarea(
            description="",
            disabled=True,
            layout=ipw.Layout(width="415px", height="250px"),
        )
        self.substance_image_box = utils.Image(
            value=utils.read_file(self.config["default_image_filepath"]),
            format="jpg",
            width="220px",
            height="250px",
        ...
        self.substance_metadata_boxes = ipw.HBox(
            [
                self.substances_dropdown_boxes,
                self.substance_details_textbox,
                self.substance_image_box,
            ]
        )

        # Get widgets for molecule dropdown
        self.molecules_dropdown_boxes = utils.DropdownwithSortingCheckboxesWidget(
            "Molecule", ipw.Layout(width="335px"), {"description_width": "110px"}, [-1]
        )
        self.molecule_details_textbox = utils.Textarea(
            description="",
            disabled=True,
            layout=ipw.Layout(width="415px", height="250px"),
        )
        self.molecule_image_box = utils.Image(
            value=utils.read_file(self.config["default_image_filepath"]),
            format="jpg",
            width="220px",
            height="250px",
        ...
        self.molecule_metadata_boxes = ipw.HBox(
            [
                self.molecules_dropdown_boxes,
                self.molecule_details_textbox,
                self.molecule_image_box,
            ]
        )

        # Get widgets for product dropdown
        self.products_dropdown_boxes = utils.DropdownwithSortingCheckboxesWidget(
            "Product", ipw.Layout(width="335px"), {"description_width": "110px"}, [-1]
        )
        self.product_details_textbox = utils.Textarea(
            description="",
            disabled=True,
            layout=ipw.Layout(width="415px", height="250px"),
        )
        self.product_image_box = utils.Image(
            value=utils.read_file(self.config["default_image_filepath"]),
            format="jpg",
            width="220px",
            height="250px",
        ...
        self.product_metadata_boxes = ipw.HBox(
            [
                self.products_dropdown_boxes,
                self.product_details_textbox,
                self.product_image_box,
            ]
        )
            ...
            for prop_key in object["properties"]:
                ...
                if property["property_widget"] == "TEXT":
                    prop_widget = utils.Text(
                        ...
                        style={
                            "description_width": property["box_layout"][
                                "description_width"
                            ]
                        },
                    )
                    ...
                    prop_widget = utils.FloatTextwithDropdownWidget(
                        property["title"],
                        ipw.Layout(width=property["box_layout"]["width"]),
                        ...
                        ipw.Layout(width=property["dropdown_layout"]["width"]),
                        property["units"],
                    ...
                    prop_widget = utils.IntSliderwithTextWidget(
                        ...
                        {
                            "description_width": property["slider_layout"][
                                "description_width"
                            ]
                        },
        ...
        self.sample_out_name_textbox = utils.Text(
            description="Name",
            disabled=False,
            layout=ipw.Layout(width="400px"),
        ...
        self.create_button = utils.Button(
            description="",
            disabled=False,
            button_style="",
            tooltip="Save",
        ...
        self.download_button = utils.Button(
            description="",
            disabled=False,
            button_style="",
            tooltip="Download",
        ...
        self.download_close_buttons_hbox = ipw.HBox(
            [self.download_button, self.quit_button]
        )

        # Sample Preparation widgets (Select multiple and Accordion)
        self.sample_prep_selector = utils.SelectMultiple(
        ...
        self.sample_prep_selector.layout = ipw.Layout(
            width="220px", height=selector_height
        )
        self.sample_prep_accordion = ipw.Accordion(children=[])

        # Widget to select folder with measurement files
        self.folder_selector = utils.FileChooser(
            path=".", select_default=True, use_dir_icons=True, show_only_dirs=True
        )

        # Selectors widgets
        self.measurements_selector = utils.SelectMultiple(
            description="Measurements",
            disabled=False,
            layout=ipw.Layout(width="800px"),
            style={"description_width": "110px"},
        )
        self.results_selector = utils.SelectMultiple(
            description="Results",
            disabled=False,
            layout=ipw.Layout(width="800px"),
            style={"description_width": "110px"},
        )
        self.drafts_selector = utils.SelectMultiple(
            description="Drafts",
            disabled=False,
            layout=ipw.Layout(width="800px"),
            style={"description_width": "110px"},
        )
        self.authors_selector = utils.SelectMultiple(
            description="Authors",
            disabled=False,
            layout=ipw.Layout(width="800px"),
            style={"description_width": "110px"},
        ...
        self.grants_selector = utils.SelectMultiple(
            description="Grants",
            disabled=False,
            layout=ipw.Layout(width="800px"),
            style={"description_width": "110px"},
        ...
        self.create_new_experiment_button.on_click(
            self.create_new_experiment_button_on_click
        )
        self.cancel_new_experiment_button.on_click(
            self.cancel_new_experiment_button_on_click
        )
        self.save_new_experiment_button.on_click(
            self.save_new_experiment_button_on_click
        )
        self.sample_prep_selector.observe(
            self.sample_preparation_widgets, names="value"
        )

        # Increase buttons icons' size
        self.increase_buttons_size = utils.HTML(
            data="".join(self.config["save_home_buttons_settings"])
        )
        display(self.increase_buttons_size)

        # Dropdown box for getting openBIS objects
        self.object_dropdown = utils.DropdownwithSortingCheckboxesWidget(
            "Object", ipw.Layout(width="982px"), {"description_width": "110px"}, [-1]
        )
    ...
    def download_openbis_object_metadata(self, b):
        if self.object_dropdown.children[0].value == -1:
            ...
            openbis_selected_object = self.openbis_session.get_sample(
                openbis_selected_object_permid
            )
    ...
    def sample_preparation_widgets(self, change):
        ...
        for i, task in enumerate(self.sample_prep_selector.value):
            ...
            if self.config["objects"][task]["uses_substance"]:
                task_widgets.append(self.substance_metadata_boxes)
    ...
    def load_measurements(self, change):
        if self.projects_dropdown_boxes.children[0].value != -1:
            ...
            measurements_list.extend(
                utils.load_openbis_elements_list(
                    self.openbis_session,
                    "1D_MEASUREMENT",
                    self.projects_dropdown_boxes.children[0].value,
                )
            )
            measurements_list.extend(
                utils.load_openbis_elements_list(
    ...
    def load_results(self, change):
        if self.projects_dropdown_boxes.children[0].value != -1:
            self.results_selector.options = utils.load_openbis_elements_list(
                self.openbis_session,
                "RESULTS",
                self.projects_dropdown_boxes.children[0].value,
            )
    ...
    def load_drafts(self, change):
        if self.projects_dropdown_boxes.children[0].value != -1:
            self.drafts_selector.options = utils.load_openbis_elements_list(
                self.openbis_session,
                "DRAFT",
                self.projects_dropdown_boxes.children[0].value,
            )
    ...
    def create_new_experiment_button_on_click(self, b):
        with self.select_experiment_output:
            clear_output()
            display_list = [
                self.experiments_dropdown_boxes,
                self.new_experiment_name_textbox,
                self.projects_dropdown_boxes,
                ipw.HBox(
                    [self.save_new_experiment_button, self.cancel_new_experiment_button]
                ),
                self.sample_metadata_boxes,
                self.instruments_dropdown_boxes,
            ]
    ...
    def save_new_experiment_button_on_click(self, b):
        utils.create_experiment_in_openbis(
            self.openbis_session,
            self.projects_dropdown_boxes.children[0].value,
            self.new_experiment_name_textbox.value,
        )
        with self.select_experiment_output:
            clear_output()
            utils.load_dropdown_elements(
                self.openbis_session,
                "EXPERIMENT",
                self.experiments_dropdown,
                self.experiment_sorting_checkboxes,
                "experiment",
            )
            display_list = [
                self.experiments_dropdown_boxes,
    ...
    def create_sample_action(self, b):
        samples_names = [
            sample.props["$name"]
            for sample in self.openbis_session.get_objects(type="SAMPLE")
        ]
        if self.sample_out_name_textbox.value in samples_names:
            ...
            sample_parents = (
                ...
                or self.material_selection_radio_button.value == "No material"
                else [self.materials_dropdown.value]
            )
    ...
    def load_material_metadata(self, change):
        if self.materials_dropdown.value == -1:
            ...
            self.material_image_box.value = utils.read_file(
                self.config["default_image_filepath"]
            )
            return

        # Get selected object properties information from config file
        selected_object = self.material_selection_radio_button.value
        selected_object_properties = self.config["objects"][selected_object][
            "properties"
        ]
        ...
        if material_dataset:
            ...
            self.material_image_box.value = utils.read_file(
                f"images/{material_dataset.permId}/{material_dataset.file_list[0]}"
            )
        ...
        for prop_key in selected_object_properties:
            ...
            if self.config["properties"][prop_key]["property_type"] == "QUANTITY_VALUE":
                ...
                if value:
                    ...
                    material_metadata_string += (
                        f"{prop_title}: {prop_dict['value']} {prop_dict['unit']}\n"
                    )
                else:
                    material_metadata_string += f"{prop_title}: {value}\n"
            else:
                material_metadata_string += (
                    f"{prop_title}: {material_metadata.get(prop_key)}\n"
                )
    ...
    def select_material_radio_change(self, change):
        ...
        for object_key, object_info in self.config["objects"].items():
            if object_info["object_type"] == "slab":
                material_types[object_key] = (
                    object_info["openbis_object_type"],
                    object_info["placeholder"],
                )
        ...
        if material_class:
            ...
            materials_names_permids = [
                (f"{mat.props['$name']} ({mat.permId})", mat.permId)
                for mat in materials
            ]
            self.materials_dropdown.options = [
                (placeholder, -1)
            ] + materials_names_permids
            self.materials_dropdown.value = -1

            utils.sort_dropdown(
                self.material_sorting_checkboxes, self.materials_dropdown
            )
            ...
            display(
                ipw.HBox([self.materials_dropdown, self.material_sorting_checkboxes])
            )
    ...
    def upload_datasets(self, method_object, task):
        for filename in self.objects_support_files_widgets[task].value:
            ...
            self.openbis_session.new_dataset(
                type="RAW_DATA", sample=method_object, files=[filename]
            ).save()
            os.remove(filename)
    ...
    def create_miscellaneous_action(self, b):
        ...
        object_properties = {
            "$name": self.property_widgets["$name"].value,
            "comments": self.property_widgets["comments"].value,
        }

        method_object = utils.create_openbis_object(
            self.openbis_session,
            type=self.config["objects"]["Miscellaneous"]["openbis_object_type"],
    ...
    def create_calibration_optimisation_action(self, b):
        ...
        if (
            self.substances_dropdown_boxes.children[0].value == -1
            and self.config["objects"][self.method_type]["uses_substance"]
        ):
        ...
        if self.config["objects"][self.method_type]["uses_substance"]:
            object_parents.append(self.substances_dropdown_boxes.children[0].value)
        ...
        for prop in self.config["objects"][self.method_type]["properties"]:
            if self.config["properties"][prop]["property_type"] == "string":
                ...
                object_properties[prop] = json.dumps(
                    {
                        "evaporator_number": self.property_widgets[prop]
                        .children[0]
                        .value,
                        "details": self.property_widgets[prop].children[1].value,
                    }
                )
            elif self.config["properties"][prop]["property_type"] == "QUANTITY_VALUE":
                object_properties[prop] = json.dumps(
                    {
                        "has_value": self.property_widgets[prop].children[0].value,
                        "has_unit": self.property_widgets[prop].children[1].value,
                    }
                )

        method_object = utils.create_openbis_object(
            self.openbis_session,
            type=self.config["objects"][self.method_type]["openbis_object_type"],
            collection=self.experiments_dropdown.value,
            props=object_properties,
    ...
    def create_sample_preparation_action(self, b):
        ...
        for i, task in enumerate(self.sample_prep_selector.value):
            method_parents = [
                self.samples_dropdown_boxes.children[0].value,
                self.instruments_dropdown_boxes.children[0].value,
            ]

            if (
                self.substances_dropdown_boxes.children[0].value == -1
                and self.config["objects"][task]["uses_substance"]
            ):
                print("Select a substance.")
                return

            if self.config["objects"][task]["uses_substance"]:
                method_parents.append(self.substances_dropdown_boxes.children[0].value)
            ...
            for prop in self.config["objects"][task]["properties"]:
                if prop == "evaporator_slot":
                    object_properties[prop] = json.dumps(
                        {
                            "evaporator_number": self.object_widgets[task][prop]
                            .children[0]
                            .value,
                            "details": self.object_widgets[task][prop]
                            .children[1]
                            .value,
                        }
                    )
                    ...
                    self.config["properties"][prop]["property_type"] == "QUANTITY_VALUE"
                ):
                    object_properties[prop] = json.dumps(
                        {
                            "has_value": self.object_widgets[task][prop]
                            .children[0]
                            .value,
                            "has_unit": self.object_widgets[task][prop]
                            .children[1]
                            .value,
                        }
                    )
            ...
            method_object = utils.create_openbis_object(
                self.openbis_session,
                type=self.config["objects"][task]["openbis_object_type"],
                collection=self.experiments_dropdown.value,
                props=object_properties,
                parents=method_parents,
            )
        ...
        parent_sample = self.openbis_session.get_object(
            self.samples_dropdown_boxes.children[0].value
        )
    ...
    def create_draft_openbis(self, b):
        ...
        project_drafts_collection = self.openbis_session.get_collections(
        ...
        if len(project_drafts_collection) == 0:
            drafts_collection = utils.create_openbis_collection(
                self.openbis_session,
                project=self.projects_dropdown_boxes.children[0].value,
    ...
    def create_results_openbis(self, b):
        ...
        project_permid = self.projects_dropdown_boxes.children[0].value
        project_results_collection = self.openbis_session.get_collections(
            project=project_permid, code="RESULTS_COLLECTION"
        )
    ...
    def load_substance_metadata(self, change):
        if self.substances_dropdown_boxes.children[0].value == -1:
            self.substance_details_textbox.value = ""
            self.substance_image_box.value = utils.read_file(
                self.config["default_image_filepath"]
            )
            return
        ...
        substance_property_list = self.config["objects"]["Molecule"]["properties"]
        material_object = self.openbis_session.get_object(
            self.substances_dropdown_boxes.children[0].value
        )

        # Get substance image
        substance_object = self.openbis_session.get_object(
            material_object.props.all()["has_molecule"]
        )
        ...
        if substance_dataset:
            ...
            self.substance_image_box.value = utils.read_file(
                f"images/{substance_dataset.permId}/{material_image_filepath}"
            )
        ...
        for prop_key in property_list:
            ...
            if (
                self.config["properties"][prop_key]["property_type"] == "QUANTITY_VALUE"
                and prop_value is not None
            ):
                prop_value = json.loads(prop_value)
                material_metadata_string += (
                    f"{prop_title}: {prop_value['value']} {prop_value['unit']}\n"
                )
                ...
                for mol_prop_key in substance_property_list:
                    ...
                    material_metadata_string += (
                        f"- {mol_prop_title}: {mol_prop_value}\n"
                    )
            else:
                material_metadata_string += f"{prop_title}: {prop_value}\n"

        self.substance_details_textbox.value = material_metadata_string
    ...
    def load_sample_metadata(self, change):
        ...
        sample_object = self.openbis_session.get_object(
            self.samples_dropdown_boxes.children[0].value
        )
        sample_parents_metadata = utils.get_openbis_parents_recursive(
            self.openbis_session, sample_object, []
        )
        ...
        while parent_idx < number_parents:
            ...
            if parent_metadata[0] == "DEPOSITION":
                ...
                for parent_id in deposition_object.parents:
                    deposition_parent_object = self.openbis_session.get_object(
                        parent_id
                    )
                    if deposition_parent_object.type == "SUBSTANCE":
                        deposition_parent_object_metadata = (
                            deposition_parent_object.props.all()
                        )
                        substance_metadata = [
                            deposition_parent_object_metadata["$name"],
                            deposition_parent_object.permId,
                        ]
            ...
            if parent_metadata[0] in self.sample_preparation_sample_types:
                if sample_metadata_string not in sample_strings["sample_preparations"]:
                    ...
                    sample_strings["sample_preparation_datetimes"].append(
                        parent_metadata[2]
                    )
        ...
        parsed_datetimes = [
            ...
            for dt in sample_strings["sample_preparation_datetimes"]
        ]
        zipped_lists = list(
            zip(parsed_datetimes, sample_strings["sample_preparations"])
        )
        ...
        sample_metadata_string = (
            f"PermId: {sample_object.attrs.permId}\nMaterial:\n"
        ...
        if last_sample_preparation:
            last_sample_preparation_object = self.openbis_session.get_object(
                last_sample_preparation
            )
            # Automatically select the experiment where the last sample preparation task was saved
            last_sample_preparation_experiment = self.openbis_session.get_experiment(
                last_sample_preparation_object.attrs.experiment
            )
            # Notify the user in case the experiment changed
            if (
                self.experiments_dropdown.value
                != last_sample_preparation_experiment.permId
                and self.experiments_dropdown.value != -1
            ):
                dropdown_options_dict = {
                    key: val for val, key in self.experiments_dropdown.options
                }
                previous_experiment_name = dropdown_options_dict.get(
                    self.experiments_dropdown.value
                )
                new_experiment_name = dropdown_options_dict.get(
                    last_sample_preparation_experiment.permId
                )
                display(
                    utils.Javascript(
                        data=f"alert('{f'Experiment was changed from {previous_experiment_name} to {new_experiment_name}!'}')"
                    )
                )
            ...
            for parent in last_sample_preparation_object.parents:
                ...
                if parent_object.type == "INSTRUMENT":
                    self.instruments_dropdown_boxes.children[
                        0
                    ].value = parent_object.permId
        ...
        for i, task in enumerate(self.sample_prep_selector.value):
            sample_out_name = (
                f"{sample_out_name}_{self.object_widgets[task]['$name'].value}"
                if self.object_widgets[task]["$name"].value
                else sample_out_name
            )
    ...
    def load_dropdown_lists(self):
        ...
        utils.load_dropdown_elements(
            self.openbis_session,
            "SAMPLE",
            self.samples_dropdown_boxes.children[0],
            self.samples_dropdown_boxes.children[1],
            "sample",
        )
        utils.load_dropdown_elements(
            self.openbis_session,
            "INSTRUMENT",
            self.instruments_dropdown_boxes.children[0],
            self.instruments_dropdown_boxes.children[1],
            "instrument",
        )
        utils.load_dropdown_elements(
            self.openbis_session,
        ...
        utils.load_dropdown_elements(
            self.openbis_session,
            "PROJECT",
            self.projects_dropdown_boxes.children[0],
            self.projects_dropdown_boxes.children[1],
            "project",
        )
        utils.load_dropdown_elements(
            self.openbis_session,
            "MOLECULE",
            self.molecules_dropdown_boxes.children[0],
            self.molecules_dropdown_boxes.children[1],
            "molecule",
        )
        utils.load_dropdown_elements(
            self.openbis_session,
            "PRODUCT",
            self.products_dropdown_boxes.children[0],
            self.products_dropdown_boxes.children[1],
            "product",
        )
        utils.load_dropdown_elements(
            self.openbis_session,
            "SUBSTANCE",
            self.substances_dropdown_boxes.children[0],
            self.substances_dropdown_boxes.children[1],
            "substance",
        )
        utils.load_dropdown_elements(
            self.openbis_session,
            "PUBLICATION_CUSTOM",
            self.object_dropdown.children[0],
            self.object_dropdown.children[1],
            "publication",
        )

    def update_text(self, change):
        if self.samples_dropdown_boxes.children[0].value != -1:
            selected_sample_name = next(
                label
                for label, val in self.samples_dropdown_boxes.children[0].options
                if val == self.samples_dropdown_boxes.children[0].value
            )
            for i, task in enumerate(self.sample_prep_selector.value):
    ...
    def upload_measurements_to_openbis(self, b):
        ...
        measurements_collection = next(
            (
                ...
                if self.openbis_session.get_object(child_id).type
                in ["1D_MEASUREMENT", "2D_MEASUREMENT"]
            ),
            None,
        )
        ...
        sample_project = next(
            (
                ...
                if self.openbis_session.get_object(parent_id).type
                in self.sample_preparation_sample_types
            ),
            None,
        )
        ...
        correct_folder = all(
            f".{filename.split('.')[-1]}" in self.measurement_file_extensions
        ...
        if correct_folder:
            if measurements_collection:
                measurements_collection = (
                    f"{sample_project.identifier}/{measurements_collection}"
                )
                measurements_collection = self.openbis_session.get_collection(
                    measurements_collection
                )
            else:
                if not sample_project:
                    ...
                    return
                collection_props = {
                    "$name": f"Measurements from Sample {sample_object.props['$name']}",
            ...
            nanonis_importer.upload_measurements_into_openbis(
                ...
                data_folder,
                measurements_collection.permId,
                sample_object.permId,
                self.instruments_dropdown_boxes.children[0].value,
            )
            print("Upload successful!")
        else:
            print(
                f"Folder contains unrecognised files. Please remove files with extensions other than: {', '.join(self.measurement_file_extensions)}."
            )
