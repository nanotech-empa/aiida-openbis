...
class ExperimentSelectionWidget(ipw.VBox):
    def __init__(self):
        ...
        self.dropdown_details = ipw.HBox(
            [self.dropdown, self.create_new_experiment_button]
        )
        self.add_experiment_output = ipw.Output()
        self.children = [
            self.dropdown_details,
            self.sorting_checkboxes_list,
            self.add_experiment_output,
        ]
    ...
    def load_dropdown_box(self):
        items = utils.get_openbis_collections(OPENBIS_SESSION, type="EXPERIMENT")
        items_names_permids = [
            (f"{item.props['name']} (Project: {item.project.code})", item.permId)
            for item in items
        ]
        items_names_permids.insert(0, ("Select experiment...", -1))
    ...
    def save_new_experiment_button_on_click(self, b):
        utils.create_experiment_in_openbis(
            OPENBIS_SESSION,
            self.projects_dropdown_boxes.dropdown.value,
...
class MaterialSelectionWidget(ipw.Output):
    def __init__(self):
        ...
        self.image_box = ipw.Image(
            ...
            format="jpg",
            layout=ipw.Layout(
                border="solid 1px #cccccc", width="220px", height="250px"
            ),
        )
    ...
    def load_dropdown_box(self, type):
        ...
        openbis_type = (
            DATA_MODEL["classes"][type]["annotations"]["openbis_label"]
            .replace(" ", "_")
            .upper()
        )
        placeholder = DATA_MODEL["classes"][type]["title"]

        items = utils.get_openbis_objects(OPENBIS_SESSION, type=openbis_type)
    ...
    def load_metadata(self, change):
        ...
        if object_dataset:
            ...
            self.image_box.value = utils.read_file(
        ...
        metadata_string = utils.get_metadata_string(
            OPENBIS_SESSION, object, self.schema_object_type, "", DATA_MODEL
        )
...
class ObjectPropertiesWidgets(ipw.VBox):
    ...
    def get_properties_widgets(self):
        ...
        for property in object["slots"]:
            if (
                self.all_schema_slots[property]["annotations"]["openbis_type"]
                != "Not used"
            ):
                ...
                if property_widget:
                    ...
                    self.properties_widgets_detailed_dict[new_property] = property_dict[
                        property
                    ]
    ...
    def get_property_widget(self, property, is_group=False):
        ...
        if property_multivalued:
            ...
            text_widget = utils.Text(
                layout=ipw.Layout(width="150px"), placeholder="", disabled=self.disabled
            )
            ...
            int_widget = utils.Text(
                layout=ipw.Layout(width="150px"), disabled=self.disabled
            )
            ...
            def validate_input(change):
                ...
                if new_value == "-" or new_value.lstrip("-").isdigit():
                    ...
                    if new_value.startswith("-"):
                        cleaned_value = (
                            "-" + cleaned_value
                        )  # Keep '-' at the start if it was originally there
            ...
            def validate_float_input(change):
                ...
                if not valid_float_pattern.fullmatch(
                    new_value
                ) and not intermediate_pattern.fullmatch(new_value):
            ...
            property_options = []
            property_vocabulary = DATA_MODEL["enums"][property_range][
                "permissible_values"
            ]
            ...
            if "slots" in self.all_schema_classes[property_range]:
                property_widget, property_dict = self.get_property_widgets_recursive(
                    property, property_range
                )
            else:
                if is_group:
                    label_widget = ipw.HTML(value=property_description)
    ...
    def get_property_widgets_recursive(self, property, property_range):
        ...
        for slot in self.all_schema_classes[property_range]["slots"]:
            if self.all_schema_slots[slot]["annotations"]["openbis_type"] != "Not used":
                sub_property_widget, sub_property_dict = self.get_property_widget(
                    slot, True
                )
                property_dict[property][slot] = sub_property_dict[slot]
                widget_accordion_children.append(sub_property_widget)
        property_widget = utils.build_dynamic_hbox(
            widget_accordion_children, max_per_row=3
        )
        property_description = self.all_schema_slots[property]["title"]

        return ipw.VBox(
            [ipw.HTML(value=f"<b>{property_description}</b>"), property_widget]
        ), property_dict
        ...
        for type in openbis_objects_types:
            if project_id:
                items = utils.get_openbis_objects(
                    OPENBIS_SESSION, type=type, project=project_id
                )
            else:
            ...
            options = [
                (f"{item.props['name']} ({item.permId})", item.permId) for item in items
            ]
...
class ObjectSelectionWidget(ipw.HBox):
    def __init__(
        self,
        type,
        widgets_types={"dropdown": {"width": "500px"}},
        checkboxes_descriptions=["Name", "Registration date"],
        disabled=False,
        contains_label=True,
    ):
        # Initialize the parent HBox
        ...
        if "dropdown" in widgets_types:
            ...
            self.sorting_checkboxes_list = ipw.HBox(
                [
                    ...
                    utils.Checkbox(
                        description=self.checkboxes_descriptions[0],
                        value=False,
                        disabled=self.disabled,
                        layout=ipw.Layout(width="initial"),
                        indent=False,
                    ),
                    utils.Checkbox(
                        description=self.checkboxes_descriptions[1],
                        value=False,
                        disabled=self.disabled,
                        layout=ipw.Layout(width="initial"),
                        indent=False,
            ...
            if contains_label:
                self.dropdown_boxes = ipw.VBox(
                    [self.label, self.dropdown, self.sorting_checkboxes_list]
                )
            else:
                self.dropdown_boxes = ipw.VBox(
                    [self.dropdown, self.sorting_checkboxes_list]
                )
        ...
        if "details" in widgets_types:
            ...
            self.details_textbox = ipw.Textarea(
                ...
                disabled=True,
                layout=ipw.Layout(
                    width=widgets_types["details"]["width"],
                    height=widgets_types["details"]["height"],
                ),
            )
        ...
        if "image" in widgets_types:
            ...
            self.image_box = ipw.Image(
                ...
                layout=ipw.Layout(
                    ...
                    width=widgets_types["image"]["width"],
                    height=widgets_types["image"]["height"],
                ),
            )
    ...
    def load_dropdown_box(self, source="openbis"):
        ...
        self.type = (
            DATA_MODEL["classes"][self.schema_object_type]["annotations"][
                "openbis_label"
            ]
            .replace(" ", "_")
            .upper()
        ...
        if source == "openbis":
            items = utils.get_openbis_objects(OPENBIS_SESSION, type=self.type)

            if self.type == "SAMPLE":
                options = [
                    (f"{item.props['name']}", item.permId)
                    for item in items
                    if item.props["exists"] == "true"
                ]
            else:
...
class OpenbisObjectWidget(ipw.VBox):
    def __init__(self, obj_type, data_model, disabled=False):
        ...
        self.disabled = disabled
        self.obj_type = self.data_model["classes"][obj_type]
        self.properties = self.get_object_properties(self.obj_type)
        ...
        for prop in self.properties:
            if prop in self.data_model["slots"]:
                ...
                prop_openbis_type = self.data_model["slots"][prop]["annotations"][
                    "openbis_type"
                ]
                prop_multivalued = self.data_model["slots"][prop]["multivalued"]
                show_label = True
                prop_widget, prop_value_widget = self.get_property_widget(
                    prop_title,
                    prop_range,
                    prop_openbis_type,
                    prop_multivalued,
                ...
                if prop_widget and prop_value_widget:
                    self.properties_widgets[prop] = {
                        "widget": prop_widget,
                        "value_widget": prop_value_widget,
                    }
    ...
    def get_property_widget(self, title, range, openbis_type, multivalued, show_label):
        widget = None
        value_widget = None
        label_widget = ipw.HTML(value=f"<b>{title}</b>")
        ...
        if openbis_type == "VARCHAR":
            ...
            value_widget = widget  # Widget with the value of the property

        elif openbis_type == "MULTILINE_VARCHAR":
            widget = utils.Textarea(
                layout=ipw.Layout(width="200px", height="100px"),
                placeholder="",
                disabled=self.disabled,
            )
            ...
            def validate_input(change):
                ...
                if new_value == "-" or new_value.lstrip("-").isdigit():
                    ...
                    widget.value = cleaned_value

            widget.observe(validate_input, names="value")
            value_widget = widget  # Widget with the value of the property
            ...
            widget.observe(validate_float_input, names="value")
            value_widget = widget  # Widget with the value of the property

        elif openbis_type == "CONTROLLEDVOCABULARY":
            property_options = []
            property_vocabulary = self.data_model["enums"][range]["permissible_values"]
            for key, item in property_vocabulary.items():
                property_options.append((item["annotations"]["openbis_label"], key))
            widget = utils.Dropdown(
                layout=ipw.Layout(width="100px"),
                options=property_options,
                value=property_options[0][1],
                disabled=self.disabled,
            ...
            if "slots" in prop_class:
                ...
                for slot in prop_slots:
                    ...
                    slot_openbis_type = self.data_model["slots"][slot]["annotations"][
                        "openbis_type"
                    ]
                    show_label = True
                    widget, value_widget = self.get_property_widget(
                        slot_title,
                        slot_range,
                        slot_openbis_type,
                        multivalued=False,
                ...
                if multivalued:
                    prop_value_widgets = [prop_value_widgets]
                    json_items_widgets = ipw.HBox(list(prop_widgets.values()))
                    add_rows_button = utils.Button(icon="plus")
                    remove_rows_button = utils.Button(icon="minus")
                    edit_table_buttons = ipw.HBox([add_rows_button, remove_rows_button])
                    json_widget = ipw.VBox([json_items_widgets, edit_table_buttons])

                    def add_row(change):
                        """Add a new row to the JSON widget."""
                        new_widgets = {}
                        new_prop_value_widgets = {}
                        for slot in prop_slots:
                            slot_title = self.data_model["slots"][slot]["title"]
                            slot_range = self.data_model["slots"][slot]["range"]
                            slot_openbis_type = self.data_model["slots"][slot][
                                "annotations"
                            ]["openbis_type"]
                            widget, value_widget = self.get_property_widget(
                                slot_title,
                            ...
                            if widget and value_widget:
                                new_widgets[slot] = widget
                                new_prop_value_widgets[slot] = value_widget

                        new_json_items_widgets = ipw.HBox(list(new_widgets.values()))
                        json_widget.children = list(json_widget.children[:-1]) + [
                            new_json_items_widgets,
                            edit_table_buttons,
                        ]
                        prop_value_widgets.append(new_prop_value_widgets)

                    def remove_row(change):
                        """Remove the last row from the JSON widget."""
                        if len(json_widget.children) > 1:
                            json_widget.children = list(json_widget.children[:-2]) + [
                                edit_table_buttons
                            ]
                            prop_value_widgets.pop()

                    add_rows_button.on_click(add_row)
                    remove_rows_button.on_click(remove_row)
                    widget = json_widget
            ...
            object_selector_config = {
                "dropdown": {"width": "250px"},
                "details": {"width": "200px", "height": "250px"},
                "image": {"width": "220px", "height": "250px"},
            }
            widget = ObjectSelectionWidget(
                range, object_selector_config, contains_label=False
            )
            widget.load_dropdown_box()
            widget.dropdown.observe(widget.load_metadata, names="value")
            value_widget = widget
    ...
    def get_values(self):
        """Get the values of the properties widgets."""
        values = {}
        for prop, item in self.properties_widgets.items():
            widget = item["value_widget"]

            def extract_value(w):
                if isinstance(w, (ipw.Text, ipw.Textarea, ipw.Checkbox, ipw.Dropdown)):
                    return w.value
                elif isinstance(w, ipw.DatePicker):
                    return w.value.isoformat() if w.value else None
                elif isinstance(w, ObjectSelectionWidget):
                    return w.dropdown.value
                return None

            if isinstance(widget, list):
                value_list = []
                for item_widget in widget:
                    item_values = {}
                    for sub_prop, sub_widget in item_widget.items():
                        sub_value = extract_value(sub_widget)
                        if sub_value not in (None, "", []):  # skip empty values
                            item_values[sub_prop] = sub_value
                    if item_values:
                        value_list.append(item_values)
                if value_list:
                    values[prop] = json.dumps(value_list)
            else:
                value = extract_value(widget)
...
class ProjectSelectionWidget(ipw.VBox):
    ...
    def load_dropdown_box(self):
        items = utils.get_openbis_projects(OPENBIS_SESSION)
        items_names_permids = [(f"{item.code}", item.permId) for item in items]
        items_names_permids.insert(0, ("Select project...", -1))
        self.dropdown.options = items_names_permids
        self.dropdown.value = -1
...
class SamplePreparationMultipleSelectionWidget(ipw.HBox):
    def __init__(self, sample_preparation_tasks):
        ...
        self.sample_preparation_options = [
            (
                DATA_MODEL["classes"][sample_preparation]["description"],
                sample_preparation,
            )
            for sample_preparation in sample_preparation_tasks
        ]
