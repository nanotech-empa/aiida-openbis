...
def show_object_in_openbis_menu(session, object_type_code: str):
    ...
    try:
        ...
        settings["sampleTypeDefinitionsExtension"][object_type_code] = {
        ...
        print(
            "The object is not available in the main menu. For that, open openBIS, go to Settings, click on Edit, open one object type in the Object Type definitions Extension and add to the main menu. Then, this function works."
        )


def assign_properties_to_object_type_in_openbis(
    session, object_type, object_property_types: list[str]
):
...
def log_in(openbis_url: str, openbis_user: str, openbis_pw: str):
    ...
    if Openbis(openbis_url, verify_certificates=False).is_token_valid():
        ...
        Openbis(openbis_url, verify_certificates=False).login(
            openbis_user, openbis_pw, save_token=True
        )
        return Openbis(openbis_url, verify_certificates=False)
...
class OpenBisDatabase:
    ...
    def create_object_type(self, object_type_details: dict):
        object_type_dict = {
            "code": object_type_details["annotations"]["openbis_label"].replace(
                " ", "_"
            ),
    ...
    def generate_vocabulary_dictionary(self, property_type: str) -> dict:
        ...
        for term, term_details in vocabularies[property_type_range][
            "permissible_values"
        ].items():
            term_dict = {
                "code": term,
    ...
    def generate_property_type_dictionary(self, property_type: str) -> dict:
        property_type_dict = {
            "code": property_type,
            "label": self.objects_schema["slots"][property_type]["annotations"][
                "openbis_label"
            ],
            "description": self.objects_schema["slots"][property_type]["description"],
            "dataType": self.objects_schema["slots"][property_type]["annotations"][
                "openbis_type"
            ],
    ...
    def create_property_type(self, property_type: str) -> str:
        if self.objects_schema["slots"][property_type]["annotations"][
            "openbis_type"
        ] not in ["Not used", "OBJECT (PARENT)", "OBJECT (CHILDREN)"]:
            ...
            if property_type_openbis:
                ...
                property_type_dict = self.generate_property_type_dictionary(
                    property_type
                )
    ...
    def setup_openbis_database(self, collections_config_filepath: str):
        ...
        documentation_website_property = self.session.get_property_types(
            ...
            if object["annotations"]["openbis_object"]:
                ...
                if "slots" in object:
                    ...
                    while "is_a" in object_copy:
                        parent_object = self.objects_schema["classes"][
                            object_copy["is_a"]
                        ]
                    ...
                    assign_properties_to_object_type_in_openbis(
                        self.session, object_type, object_property_types
                    )
            ...
            if "projects" in space_info:
                for project_code, project_info in space_info["projects"].items():
                    create_project_in_openbis(
                        self.session, space_code, project_code, project_info
                    )
...
if __name__ == "__main__":
    ...
    if not root_folder_path:
        ...
        print(
            "Usage: python3 setup_openbis_using_linkml.py -o <OPENBIS_URL> -u <OPENBIS_USER> -pw <OPENBIS_PW> -r <ROOT_FOLDER>"
        )
        ...
        schema_filepath = os.path.join(
            root_folder_path, "Metadata_Schemas_LinkML", "materialMLinfo.yaml"
        )
        ...
        collections_config_filepath = os.path.join(
            root_folder_path, "scripts", "collections_config.json"
        )
