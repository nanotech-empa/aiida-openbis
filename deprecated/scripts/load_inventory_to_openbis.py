...
def upload_molecules_to_openbis(session, datasets_folderpath):
    ...
    structures_folderpath = os.path.join(
        datasets_folderpath, "molecules_dataset", "structures"
    )
    structures_workshop_folderpath = os.path.join(
        datasets_folderpath, "molecules_dataset", "structures_workshop"
    )
    structures_filepaths = get_full_filepath(structures_folderpath) + get_full_filepath(
        structures_workshop_folderpath
    )
    ...
    for _, row in data.iterrows():
        ...
        molecule_concept_object = get_openbis_object_by_name(
            session, "MOLECULE_CONCEPT", molecule_series_number
        )
        ...
        if molecule_concept_object:
            ...
            molecule_cdxml_filepath = find_cdxml_file(
                structures_filepaths, molecule_series_number
            )
            ...
            if molecule_cdxml_filepath:
                ...
                if len(molecules) == 1:
                    ...
                    print(
                        f"There are more than one molecule in the file: {molecule_cdxml_filepath}"
                    )
                else:
                    print(
                        f"There are no molecules in the file: {molecule_cdxml_filepath}"
                    )
            ...
            if molecule_cdxml_filepath:
                session.new_dataset(
                    type="RAW_DATA",
                    object=molecule_concept_object,
                    file=molecule_cdxml_filepath,
                ).save()
            if molecule_image_filepath:
                session.new_dataset(
                    type="ELN_PREVIEW",
                    object=molecule_concept_object,
                    file=molecule_image_filepath,
                ).save()
...
def upload_instruments_to_openbis(session, folderpath):
    ...
    instruments_data = pd.read_excel(
        data_filepath, header=0, dtype=str, sheet_name="Instrument"
    )
    instruments_data = instruments_data.fillna("")
    components_data = pd.read_excel(
        data_filepath, header=0, dtype=str, sheet_name="Component"
    )
    ...
    create_json(
        instruments_metadata,
        "/home/jovyan/aiida-openbis/Notebooks/connection_to_openbis/instruments_config.json",
    )
...
def upload_inventory_to_openbis(session, folderpath):
    ...
    institutions_data = pd.read_excel(
        data_filepath, header=0, dtype=str, sheet_name="Institution"
    )
    institutions_data = institutions_data.fillna("")

    persons_data = pd.read_excel(
        data_filepath, header=0, dtype=str, sheet_name="Person"
    )
    persons_data = persons_data.fillna("")

    protocols_data = pd.read_excel(
        data_filepath, header=0, dtype=str, sheet_name="Protocol"
    )
    protocols_data = protocols_data.fillna("")

    chemical_concepts_data = pd.read_excel(
        data_filepath, header=0, dtype=str, sheet_name="ChemicalConcept"
    )
    chemical_concepts_data = chemical_concepts_data.fillna("")

    chemicals_data = pd.read_excel(
        data_filepath, header=0, dtype=str, sheet_name="Chemical"
    )
    chemicals_data = chemicals_data.fillna("")

    crystal_concepts_data = pd.read_excel(
        data_filepath, header=0, dtype=str, sheet_name="CrystalConcept"
    )
    crystal_concepts_data = crystal_concepts_data.fillna("")

    crystals_data = pd.read_excel(
        data_filepath, header=0, dtype=str, sheet_name="Crystal"
    )
    crystals_data = crystals_data.fillna("")

    reaction_product_concepts_data = pd.read_excel(
        data_filepath, header=0, dtype=str, sheet_name="ReactionProductConcept"
    )
    reaction_product_concepts_data = reaction_product_concepts_data.fillna("")

    reaction_products_data = pd.read_excel(
        data_filepath, header=0, dtype=str, sheet_name="ReactionProduct"
    )
    reaction_products_data = reaction_products_data.fillna("")

    wafer_substrate_concepts_data = pd.read_excel(
        data_filepath, header=0, dtype=str, sheet_name="WaferSubstrateConcept"
    )
    wafer_substrate_concepts_data = wafer_substrate_concepts_data.fillna("")

    wafer_substrates_data = pd.read_excel(
        data_filepath, header=0, dtype=str, sheet_name="WaferSubstrate"
    )
    wafer_substrates_data = wafer_substrates_data.fillna("")

    twod_material_concepts_data = pd.read_excel(
        data_filepath, header=0, dtype=str, sheet_name="TwoDLayerMaterialConcept"
    )
    twod_material_concepts_data = twod_material_concepts_data.fillna("")

    twod_materials_data = pd.read_excel(
        data_filepath, header=0, dtype=str, sheet_name="TwoDLayerMaterial"
    )
    ...
    for idx, row in persons_data.iterrows():
        ...
        props = {
            "name": row["name"],
            "institutions": [
                institutions_objects[institution_index - 1].permId
            ],  # 0-index correction
        }
        object = session.new_sample(
            type="PERSON",
    ...
    for idx, row in crystal_concepts_data.iterrows():
        props = {"name": row["name"], "material": row["material"], "face": row["face"]}
        object = session.new_sample(
    ...
    for idx, row in crystals_data.iterrows():
        ...
        object = session.new_sample(
            type="CRYSTAL", experiment="/MATERIALS/CRYSTALS/AU_COLLECTION", props=props
        )
        object.save()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--datasets_folderpath",
        type=str,
        help="Directory with inventory information",
        default=None,
    )
