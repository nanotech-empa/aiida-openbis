...
def create_sxm_dataset(session, experiment, file_path, sample=None, dataset_type = "IMAGING_DATA"):
    img = spm(file_path)
...
def create_dat_dataset(session, experiment, sample, folder_path, dataset_type = "IMAGING_DATA"):
    data = spm.importall(folder_path, 'spec')
    ...
    for dat_file in data:
        ...
        color_scale_visibility_x += [imaging.ImagingDataSetControlVisibility(
            "Channel X", [channel_x], [str(minimum_x), str(maximum_x), str(step_x)], channel_x_unit
        )]
        ...
        color_scale_visibility_y += [imaging.ImagingDataSetControlVisibility(
            "Channel Y", [channel_y], [str(minimum_y), str(maximum_y), str(step_y)], channel_y_unit
        )]
    ...
    inputs = [
        imaging.ImagingDataSetControl('Channel X', "Dropdown", values=[channel for channel in channels_x]),
        imaging.ImagingDataSetControl('Channel Y', "Dropdown", values=[channel for channel in channels_y]),
