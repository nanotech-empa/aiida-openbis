...
class ReacProdConceptArgs(OpenBISObjectArgs):
    sum_formula: Optional[str] = Field(default = "", description = "Molecular sum formula, e.g. 'CH4'")
    molecules: Optional[List[MoleculeArgs]] = Field(default_factory = list, description = "A list of one or more molecules that describe the reaction product.")

class ReacProdArgs(OpenBISObjectArgs):
    reacprod_concept: Optional[ReacProdConceptArgs] = Field(default_factory = list, description = "Reaction product theoretical concept.")
...
def crystal_found(obj, crystal):
    ...
    if crystal.crystal_concept.face and crystal.crystal_concept.face != crystal_concept_props.get("face"):
        return False
    if crystal.crystal_concept.material and crystal.crystal_concept.material != crystal_concept_props.get("material"):
        return False
    ...
    if substance.molecules:
        ...
        for prompt_molecule in substance.molecules:
            ...
            for molecule_permId in obj_molecules:
                ...
                if prompt_molecule.smiles and prompt_molecule.smiles != molecule_props.get("smiles"):
                    continue
                if prompt_molecule.sum_formula and prompt_molecule.sum_formula != molecule_props.get("sum_formula"):
                    continue
                if prompt_molecule.iupac_name and prompt_molecule.iupac_name != molecule_props.get("iupac_name"):
                    continue
...
def reacprod_found(obj, reac_prod):
    ...
    if reac_prod.reacprod_concept:
        ...
        if reac_prod.reacprod_concept.sum_formula:
            if reacprod_props.get("sum_formula") != reac_prod.reacprod_concept.sum_formula:
                return False
        ...
        if reac_prod.reacprod_concept.molecules:
            ...
            for prompt_mol in reac_prod.reacprod_concept.molecules:
                ...
                for mol_id in reacprod_concept_molecules:
                    ...
                    if prompt_mol.empa_number and prompt_mol.empa_number != mol_empa_number:
                        continue
                    if prompt_mol.smiles and prompt_mol.smiles != mol_props.get("smiles"):
                        continue
                    if prompt_mol.sum_formula and prompt_mol.sum_formula != mol_props.get("sum_formula"):
                        continue
                    if prompt_mol.iupac_name and prompt_mol.iupac_name != mol_props.get("iupac_name"):
                        continue
...
def create_vectorDB(db_path, embedding_model, chunks):
    if os.path.exists(db_path):
        ...
        vectordb = Chroma.from_documents(chunks, embedding_model, persist_directory=db_path)
...
# Simulations
@tool
def get_simulations_by_crystal_concept(crystal_concept: CrystalConceptArgs) -> List[str]:
    """
    ...
    if crystal_concept.permId:
        ...
        if len(objects) == 0:
            ...
            for obj in objects:
                objects_data.append(f"Crystal concept {obj.props['name']} ({obj.permId}).")
            return objects_data
...
@tool
def get_simulations_by_reacprod_concept(reacprod_concept: ReacProdConceptArgs) -> List[str]:
    """
    ...
    if reacprod_concept.permId:
        ...
        if len(reacprod_concept_objects) == 0:
            ...
            for obj in reacprod_concept_objects:
                objects_data.append(f"Reaction product concept {obj.props['name']} ({obj.permId}).")
            return objects_data
            ...
            for parent in current_obj.parents:
                ...
                if parent_obj.type.code == "CRYSTAL":
                    if input_crystal_obj.permId == parent_obj.permId:
                        objects_data.append(f"The sample {original_sample_name} ({original_sample_permid}) was made with the Crystal {parent_name} ({parent_id}).")
                        stack = []
                        break
                else:
...
def get_processes_documents(query):
    ...
    objects = openbis_utils.get_openbis_objects(type = openbis_type, props = ["description"])
    ...
    for doc in docs:
        ...
        obj_name = obj.props.name or ""
        obj_description = obj.props.description or ""
        objects_data.append(f"Process named {obj_name} described by {obj_description} ({obj_permid})")
