...
class ImportSimulationsWidget(ipw.VBox):
    def __init__(self, openbis_session):
        ...
        material_type_options = [(key, value) for key, value in MATERIALS_CONCEPTS_TYPES.items()]
    ...
    def search_simulations(self, b):
        ...
        if logical_operator == "OR": # In OR, all the simulations found are added to the list
            for parent in parents_permid_list:
                ...
                simulation_objects_children = utils.find_openbis_simulations(
                    self.openbis_session, parent_object,
                    SIMULATION_TYPES
                )
        ...
        for simulation_permid in simulation_permid_list:
            simulation_object = utils.get_openbis_object(self.openbis_session, sample_ident = simulation_permid)
    ...
    def add_molecule(self, b):
        ...
        molecule_widget = widgets.MoleculeWidget(self.openbis_session, self.molecules_accordion, molecule_index)
    ...
    def add_reacprod_concept(self, b):
        ...
        reacprod_concept_widget = widgets.ReacProdConceptWidget(self.openbis_session, self.reacprod_concepts_accordion, reacprod_concept_index)
