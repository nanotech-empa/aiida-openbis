...
def upload_instruments_to_openbis(session, folderpath, save_config_folderpath):
    ...
    instruments_data = pd.read_excel(data_filepath, header = 0, dtype=str, sheet_name = "Instruments")
    instruments_data = instruments_data.fillna('')
    components_data = pd.read_excel(data_filepath, header = 0, dtype=str, sheet_name = "Components")
    ...
    create_json(instruments_metadata, f"{save_config_folderpath}/instruments_config.json")
...
if __name__ == "__main__":
    ...
    parser.add_argument('-dir', '--instruments_folderpath', type=str, help='Directory with instrument information', default = None)
    parser.add_argument('-save_dir', '--save_config_folderpath', type=str, help='Directory to save instruments config file', default = "/home/jovyan/aiida-openbis/Notebooks/connection_to_openbis")
    ...
    upload_instruments_to_openbis(openbis_session, instruments_folderpath, save_instruments_config_folderpath)