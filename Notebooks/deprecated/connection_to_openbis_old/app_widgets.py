...
class AppWidgets():
    def __init__(self, config_filename):
        ...
        self.substances_dropdown_boxes = utils.DropdownwithSortingCheckboxesWidget('Substance', ipw.Layout(width = '335px'), {'description_width': "110px"}, [-1])
    ...
    def create_calibration_optimisation_action(self, b):
        ...
        if self.substances_dropdown_boxes.children[0].value == -1 and self.config["objects"][self.method_type]["uses_substance"]:
    ...
    def create_sample_preparation_action(self, b):
        ...
        for i, task in enumerate(self.sample_prep_selector.value):
            ...
            if self.substances_dropdown_boxes.children[0].value == -1 and self.config["objects"][task]["uses_substance"]:
                print("Select a substance.")
                return
    ...
    def load_substance_metadata(self, change):
        if self.substances_dropdown_boxes.children[0].value == -1:
            ...
            self.substance_image_box.value = utils.read_file(self.config["default_image_filepath"])
            return
        ...
        material_object = self.openbis_session.get_object(self.substances_dropdown_boxes.children[0].value)
        
        # Get substance image
        substance_object = self.openbis_session.get_object(material_object.props.all()['has_molecule'])
        ...
        if substance_dataset:
            ...
            self.substance_image_box.value = utils.read_file(f"images/{substance_dataset.permId}/{material_image_filepath}")
