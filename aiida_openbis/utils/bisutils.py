...
def log_in(bisurl="openbis", bisuser="admin", bispasswd="123456789"):
...
def new_chem_sketch(session=None, attachment=None):
    if not attachment:
        return False
    obj = session.new_object(
        collection="/MATERIALS/SAMPLES/CHEMSKETCH", type="CHEMSKETCH"
    )
...
def new_optimized_geo(session=None, pk=None):
    ...
    obj = session.new_object(
        collection="/MATERIALS/SAMPLES/COMPUTEDGEO", type="STRUCTUREDATA"
    )
...
def get_opt_geo_ids(session=None):
    return [
        (obj.permId, obj.props["aiida.uuid"])
...
def new_molecule(session=None, name=None, molid=None, smiles=None, cdxml=None):
    ...
    obj = session.new_object(
        collection="/MATERIALS/MOLECULES/MOLECULE_COLLECTION", type="MOLECULE"
    )
    obj.props["$name"] = name
...
def new_product(
    ...
    obj = session.new_object(
        collection="/MATERIALS/SAMPLES/PRODUCTS", type="MOLECULE_PRODUCT"
    )
    obj.props["name"] = name
...
def new_reaction_products(reactions=None, molecules=None, attachment=None):
    ...
    allm = {m["name"] for m in molecules} - {p["product"] for p in reactions}
    allobj = {}
...
def aiidalab_geo_opt(
    ...
    newgeoopt.props["aiida.server_url"] = (
...
def aiidalab_spm(
    zip_path=None, pk=None, collection="/DEFAULT_LAB_NOTEBOOK/7-AGNR/PRISTINE"
    ...
    newspm.props["aiida.server_url"] = (
        "https://aiidalab.materialscloud.org/user/" + node.user.email
    )
    ...
    for filename in thezip.namelist():
        if filename.endswith(".png"):
            ...
            with open(pngfile, "wb") as newf:
                ...
                rawds = session.new_dataset(
                    type="RAW_DATA", object=newspm, file=pngfile
                )
